...
5. **Walk-Forward Validation**: Models are trained using `TimeSeriesSplit(n_splits=5)` expanding-window walk-forward validation. Uncertainty bounds scale with forecast horizon using $\text{scaled\_std} = \sigma_{\text{residual}} \times \sqrt{h}$.
