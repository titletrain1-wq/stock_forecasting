...
def verify_real_data():
    ...
    with Session(engine) as session:
        ...
        # Get unique tickers
        tickers = set(b.ticker for b in bars)
        expected_tickers = {"BTC-USD", "ETH-USD", "SOL-USD", "AVAX-USD", "XRP-USD"}
...
def verify_forecasting():
    ...
    with Session(engine) as session:
        ...
        if snapshots:
            snapshot_horizons = set(s.horizon for s in snapshots)
            result["horizons_in_db"] = sorted(snapshot_horizons)
            has_bounds = all(
                s.predicted_price is not None and
                s.lower_bound is not None and
                s.upper_bound is not None
    ...
    with Session(engine) as session:
        ...
        if accuracy_records:
            result["sample_record"] = {
                ...
                "evaluated_at": accuracy_records[0].evaluated_at.isoformat() if accuracy_records[0].evaluated_at else None,
            }

        # Check if evaluator module exists and has test
        try:
            from stock_forecasting import evaluator
        ...
        if accuracy_records:
            ...
            return "WARN", "No accuracy records yet (may be OK if no forecasts matured)", result
